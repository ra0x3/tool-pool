...
async fn test_structured_return_conversion() {
    ...
    let result: Result<CallToolResult, mcpkit_rs::ErrorData> =
        mcpkit_rs::handler::server::tool::IntoCallToolResult::into_call_tool_result(structured);
...
async fn test_output_schema_requires_structured_content() {
    ...
    let call_result: Result<CallToolResult, mcpkit_rs::ErrorData> =
