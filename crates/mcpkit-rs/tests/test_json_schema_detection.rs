...
impl TestServer {
    ...
    pub async fn result_with_json(&self) -> Result<Json<TestData>, mcpkit_rs::ErrorData> {
        Ok(Json(TestData {
            value: "test".to_string(),
        }))
    }
    ...
    #[tool(name = "explicit-schema", output_schema = mcpkit_rs::handler::server::tool::schema_for_type::<TestData>())]
